...
class EditBook extends React.Component<Props, State> {
  ...
  onSubmit(values) {
    ...
    const file = this.state.previewImage.name
      ? this.state.previewImage.name
      : bookData.book.picture
    updateBook({
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      this.isErrorDecision(prevProps.bookData.error)
    ) {
      setTimeout(() => this.props.history.push('/'), 1000)
    }
    if (
      prevProps.bookData !== this.props.bookData &&
      this.props.bookData.book
    ) {
      const files = this.props.bookData.book.picture
        ? { picture: this.props.bookData.book.picture }
        : {}
      this.setState({
        previewImage: files
      })
  ...
  render() {
    ...
    return (
      <UpdateContent
        ...
        card={{
          title: bookData.book ? `${bookData.book.name}` : 'no-title',
          subtitle: '本の編集をします'
        }}
